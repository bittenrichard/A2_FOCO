...
import { GripVertical, CalendarPlus, Mail, CalendarDays, UserCheck } from 'lucide-react';
...
const ProfileTag: React.FC<{ profile: string }> = ({ profile }) => {
    const profileStyles: { [key: string]: string } = {
        Executor: 'bg-red-100 text-red-800',
        Comunicador: 'bg-yellow-100 text-yellow-800',
        Planejador: 'bg-green-100 text-green-800',
        Analista: 'bg-blue-100 text-blue-800',
    };
    const style = profileStyles[profile] || 'bg-gray-100 text-gray-800';
    return <div className={`px-2 py-0.5 rounded-full text-xs font-semibold ${style}`}>{profile}</div>;
};
    ...
    >
      ...
      <div className="p-4 cursor-pointer" onClick={() => onViewDetails(candidate)}>
        <div className="flex justify-between items-start mb-2">
            <h4 className="font-bold text-gray-900 pr-4">{candidate.nome}</h4>
        ...
        <div className="flex items-center gap-2 mb-2">
            {candidate.perfil_comportamental && <ProfileTag profile={candidate.perfil_comportamental} />}
        </div>
