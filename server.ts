...
app.get('/api/assessment/result/:assessmentId', async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (results && results.length > 0) {
            ...
            // Tenta fazer o parse do campo 'analise_ia' aqui no backend
            if (profileData.analise_ia && typeof profileData.analise_ia === 'string') {
                try {
                    profileData.analise_ia = JSON.parse(profileData.analise_ia);
                } catch (e) {
                    console.error("Erro ao fazer parse da análise da IA no backend, enviando como null:", e);
                    profileData.analise_ia = null; // Se falhar, envia null para o frontend
                }
            }
...
Olá! Analisei o novo erro que você enviou. A tela branca acompanhada da mensagem `TypeError: t.toFixed is not a function` no console é um sinal claro de que estamos tentando usar uma função de número em algo que não é um número.

Isso acontece por uma pequena falha na forma como os dados são passados entre o backend e o frontend. Especificamente, o backend está enviando o campo `analise_ia` já como um objeto, mas o frontend está tentando interpretá-lo como texto, o que causa a confusão.

Vamos resolver isso com um ajuste preciso no backend, para garantir que ele sempre envie os dados no formato correto, e uma pequena correção no frontend para ele receber esses dados da maneira certa.

### Passo 1: Ajustar o Backend

Vamos garantir que o backend sempre envie a `analise_ia` como um objeto, fazendo o parse antes de enviar.
