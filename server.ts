...
app.post('/api/assessment/:assessmentId/submit', async (req: Request, res: Response) => {
    ...
    try {
        ...
        // <-- PROMPT ATUALIZADO PARA GERAR ANÁLISE COMPLETA -->
        const prompt = `
# INSTRUÇÃO
...
Com certeza, meu nobre desenvolvedor! Sua visão está claríssima. Vamos refinar e aprofundar a análise comportamental para que ela se torne uma ferramenta poderosa e visualmente rica, tanto no modal de detalhes quanto na página de resultados, seguindo a sua referência.

A missão é clara: mais detalhes, mais visualização, mais insights. Vamos ao trabalho!

### Panorama Geral da Evolução Final

Para implementar esta versão mais completa e detalhada, seguiremos estes passos:

1.  **Enriquecer a IA (Backend):** Vamos treinar nossa IA para fornecer uma análise ainda mais profunda. No arquivo `server.ts`, vamos expandir o *prompt* para que a IA gere também **"subcaracterísticas"** e **"indicadores situacionais"** (como "Exigência do Meio", "Aproveitamento", etc.), replicando a riqueza de detalhes do seu anexo de referência.
2.  **Modernizar a Página de Resultados (Frontend):** O arquivo `AssessmentResultPage.tsx` será completamente reestruturado. Adicionaremos **barras de progresso** para visualizar facilmente a intensidade de cada perfil, além de novos componentes para exibir as subcaracterísticas e os indicadores situacionais, criando um relatório visualmente impressionante.
3.  **Potencializar o Modal de Detalhes (Frontend):** O arquivo `CandidateDetailModal.tsx` receberá as mesmas melhorias. A análise comportamental dentro do modal deixará de ser apenas um gráfico para se tornar um resumo completo, com as barras de progresso e as listas de pontos fortes e de atenção, permitindo uma tomada de decisão rápida e informada sem sair da tela de triagem.

---

### Passo 1: Expandindo a Inteligência do Backend

Vamos começar instruindo nossa IA a realizar uma análise mais complexa e a retornar os novos campos de dados que precisamos.

#### **Local do Arquivo**

Substitua todo o conteúdo do arquivo a seguir:

* `server.ts`

#### **Código Completo para `server.ts`**
...
app.get('/api/google/auth/connect', (req: Request, res: Response) => {
  const { userId } = req.query;
  if (!userId) return res.status(400).json({ error: 'userId é obrigatório' });
  const scopes = ['[https://www.googleapis.com/auth/calendar.events](https://www.googleapis.com/auth/calendar.events)'];
  const url = oauth2Client.generateAuthUrl({ access_type: 'offline', scope: scopes, prompt: 'consent', state: userId.toString() });
  res.json({ url });
});
